...
<div class="container-fluid py-4">
    <div class="row">
        <div class="col-12">
            ...
            {{-- ========================================== --}}
            {{-- کارت زیبای اطلاعات قلک (بخش جدید اضافه شده) --}}
            {{-- ========================================== --}}
            <div class="card border-0 shadow-sm mb-4" style="border-radius: 15px; background: linear-gradient(135deg, #0d6efd 0%, #0dcaf0 100%);">
                <div class="card-body p-4 text-white">
                    <div class="d-flex flex-column flex-md-row justify-content-between align-items-center">
                        <div class="d-flex align-items-center mb-3 mb-md-0">
                            <div class="bg-white bg-opacity-25 p-3 rounded-circle me-3 d-flex justify-content-center align-items-center" style="width: 65px; height: 65px;">
                                <i class="bi bi-box-seam fs-2"></i>
                            </div>
                            <div>
                                <h4 class="mb-2 fw-bold text-white">تخلیه صندوق کد: {{ $boxAllocation->box->code ?? 'نامشخص' }}</h4>
                                <div class="text-white opacity-75" style="font-size: 0.95rem;">
                                    <i class="bi bi-person-fill me-1"></i> مشترک: <strong class="text-white">{{ $boxAllocation->applicant_name }}</strong>
                                    <span class="mx-2">|</span>
                                    <i class="bi bi-info-circle-fill me-1"></i> نوع: <strong class="text-white">{{ ($boxAllocation->box->type ?? '') == 'plastic' ? 'پلاستیکی (پس از تخلیه باطل می‌شود)' : 'شیشه‌ای (پس از تخلیه موجود می‌شود)' }}</strong>
                                </div>
                            </div>
                        </div>
                        <div class="text-end">
                            <div class="bg-white text-primary px-4 py-2 rounded-pill fw-bold shadow-sm d-inline-block">
                                <i class="bi bi-receipt me-1"></i> سریال فیش بعدی: {{ $nextSerialInfo ?? '---' }}
                            </div>
                        </div>
                    </div>
                </div>
            ...
            <div class="card border-0 shadow-sm" style="border-radius: 15px;">
                ...
                <div class="card-header bg-white py-3 d-flex justify-content-between align-items-center" style="border-bottom: 1px solid #f0f0f0; border-radius: 15px 15px 0 0;">
                    <h5 class="mb-0 fw-bold text-dark">
                        <i class="bi bi-clipboard-check me-2 text-success"></i>فرم ثبت مبلغ تخلیه و صدور فیش
                    </h5>
                    {{-- نمایش تاریخ امروز --}}
                ...
                <div class="card-body p-4">
                    ...
                    <form action="{{ route('employee.boxes.update', $boxAllocation->id) }}" method="POST">
                        ...
                        <div class="row g-4 mb-4">
                            {{-- نام خیر (پیش‌فرض با نام صاحب صندوق پر می‌شود) --}}
                            <div class="col-lg-8 col-md-12">
                                <label class="form-label fw-bold text-secondary small">نام و نام خانوادگی خیر (صاحب صندوق) <span class="text-danger">*</span></label>
                                <input type="text" class="form-control form-control-lg bg-light border-0" name="donor_name" value="{{ old('donor_name', $boxAllocation->applicant_name) }}" placeholder="نام خیر را وارد کنید..." required style="height: 60px;">
                            </div>
                        ...
                        <div class="row g-4 mb-4">
                            ...
                            <div class="col-lg-8">
                                ...
                                <div class="row g-2" id="helpTypeContainer">
                                    {{-- گزینه‌های ثابت --}}
                                    @php $helpTypes = ['زکات', 'صدقه', 'فطریه', 'فرزاندن نیکان', 'قربانی']; @endphp

                                    @foreach($helpTypes as $index => $type)
                                    <div class="col-4 col-md-2">
                                        {{-- برای قلک، "صدقه" را به عنوان پیش‌فرض انتخاب می‌کنیم --}}
                                        <input type="radio" class="btn-check help-type-radio" name="help_type" id="type_{{$index}}" value="{{ $type }}" {{ old('help_type', 'صدقه') == $type ? 'checked' : '' }} required onchange="toggleOtherSelect(false)">
                            ...
                            <div class="col-lg-4">
                                ...
                                <div class="bg-light p-3 rounded-3 d-flex justify-content-around align-items-center" style="height: 66px;">
                                    ...
                                    <div class="form-check">
                                        {{-- برای قلک به صورت پیش‌فرض پرداخت موردی انتخاب است --}}
                        ...
                        <div class="row g-4 mb-4">
                            {{-- مبلغ --}}
                            <div class="col-lg-6">
                                <label class="form-label fw-bold text-secondary small">مبلغ شمارش شده (ریال) <span class="text-danger">*</span></label>
                                <div class="input-group input-group-lg">
                                    {{-- فیلد نمایشی (بدون name تا به کنترلر نرود و خطا ندهد) --}}
                                    <input type="text" inputmode="numeric" class="form-control fw-bold text-primary border-primary" id="amountInput" placeholder="0" onkeyup="processAmount(this)" required style="height: 55px; font-size: 1.2rem; direction: ltr;" autofocus>
                                    <span class="input-group-text bg-primary text-white border-primary">ریال</span>
                                </div>
                                {{-- فیلد مخفی که عدد خالص را به دیتابیس می‌فرستد --}}
                            ...
                            <div class="col-lg-6">
                                ...
                                <div class="form-text mt-1 text-muted"><i class="bi bi-info-circle me-1"></i>در صورت وارد کردن، پیامک پیام تشکر و صدور فیش ارسال می‌شود.</div>
                            </div>
                        </div>
                        ...
                        <div class="row mb-5">
                            <div class="col-12">
                                ...
                                <textarea class="form-control bg-light-subtle border-0" name="description" rows="4" placeholder="توضیحات تکمیلی، بابت، نیت خیر و ..." style="resize: none;">{{ old('description', 'بابت تخلیه صندوق کد ' . ($boxAllocation->box->code ?? '')) }}</textarea>
                            </div>
                        </div>

                        {{-- دکمه‌های ثبت و انصراف --}}
                        <div class="d-grid gap-2 d-md-flex justify-content-md-end">
                            <a href="{{ route('employee.boxes.index') }}" class="btn btn-secondary btn-lg py-3 px-5 fw-bold shadow-sm rounded-3">
                                انصراف و بازگشت
                            </a>
                            <button type="submit" class="btn btn-success btn-lg py-3 px-5 fw-bold shadow-sm rounded-3">
                                <i class="bi bi-check-circle-fill me-2"></i> ثبت نهایی و صدور فیش
                            </button>
                        </div>
...
<script>
    ...
    function processAmount(input) {
        ...
        // فرمت با کاما برای نمایش
        input.value = parseInt(rawValue).toLocaleString();
        
        // قرار دادن عدد خالص بدون کاما در فیلد مخفی برای ارسال به دیتابیس
        document.getElementById('hidden_amount').value = rawValue;
    ...
    document.addEventListener("DOMContentLoaded", function() {
        ...
        // بارگذاری مقدار اولیه (اگر صفحه بعد از ارور رفرش شده باشد)
        let hiddenAmount = document.getElementById('hidden_amount').value;
        if(hiddenAmount) {
            let amountInput = document.getElementById('amountInput');
            amountInput.value = hiddenAmount;
